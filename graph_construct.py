...
def gen_rp2ap_adjacent(pos_rssi_dict, thres_q, r0, n):
    new_pos_rssi_dict = {}
    all_aps = set()
    ...
    for rp, ap_list in new_pos_rssi_dict.items():
    ...
    non_ninf_adjacent_elem = [1 / (1 + np.power(10, np.abs(rssi - r0) / (10 * n))) for rssi in non_ninf_elem] # LDPL model
...
def gen_adjacent_matrix(pos_rssi_dict, args):
    rp2rp_matrix = gen_rp2rp_adjacent(list(pos_rssi_dict.keys()), args.thres_k)
    rp2ap_matrix = gen_rp2ap_adjacent(pos_rssi_dict, args.thres_q, args.r0, args.n)
    O = np.zeros(rp2rp_matrix.shape)
    A = np.block([rp2ap_matrix, rp2ap_matrix], [rp2ap_matrix.T, O])
    return A
